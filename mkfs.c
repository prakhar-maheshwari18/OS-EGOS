...
/* CHANGED: Increased buffer size to SIZE_2MB to prevent overflow with many apps */
char bin_dir[SIZE_2MB] = "./   6 ../   0 ";
char* contents[]  = {
    "./   0 ../   0 home/   1 bin/   8 README   5 FILEA   6 FILEB   7 ",
    "./   1 ../   0 yunhao/   2 rvr/   3 yacqub/   4 ",
    "./   2 ../   1 README   5 ",
    "./   3 ../   1 ",
    "./   4 ../   1 ",
    "This is line 1.\n"
    "This is line 2 containing the word OS.\n"
    "This is line 3.\n"
    "This is line 4 also containing the word OS.\n"
    "This is line 5.\n",
    "I am file A.\n",
    "I am file B.\n",
    bin_dir};
...
int main() {
    ...
    for (uint ino = 0; ino < BIN_DIR_INODE; ino++) {
        ...
        /* CHANGED: Use strcpy to allow full buffer usage and loop for blocks */
        strcpy(inode, contents[ino]);
        for (uint b = 0; b * BLOCK_SIZE < strlen(inode); b++) {
    ...
    /* CHANGED: Loop to write bin_dir to handle multiple blocks */
    for (uint b = 0; b * BLOCK_SIZE < strlen(bin_dir); b++) {
        filesys->write(filesys, BIN_DIR_INODE, b, (void*)(bin_dir + b * BLOCK_SIZE));
    }
